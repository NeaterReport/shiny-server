...
Transition to Value Quadrant (Text)
=======================================================================
...
### Ready for Improvment (RFI) (Motivated to act, limited ability)
...
g <- ggplot(df, aes(as.factor(`Priority Rank`), fakeheight, fill = highlight,
  ...
  facet_grid(group~.) +
  geom_text(aes(label = QuestionCode), size = 2, nudge_y = -5) +
  theme_minimal() +
  theme(legend.position="none",
...
#                                  "<br>Statement: ", Statement,
#                                  "<br>Quadrant Rank: ", `Priority Rank`))) +
#   geom_point(size = 7, alpha = 2/3, shape = 22) +
...
#         axis.text = element_text(colour = "darkgrey"),
#         axis.title.x = element_text(colour = "darkgrey")) +
#   scale_x_continuous(breaks=seq_along(1:maxrank), labels = axis_label) +
#   labs(x = "Recommended Priorities by Quadrant and Driver") +
#   scale_fill_manual(values = c("#f7f7f7", "#a8ddb5"))
...
output$graph_resp <- renderPlotly({
  ...
  geom_text(aes(label = scales::percent(percent), y = pos)) +
  theme_minimal() +
...
renderDataTable({
  ...
  df <- filter(master_comment_2016, Organization == input$org_comment)
  
  df_long <- tidyr::gather(df, Section, Comment, CMTXT:FTRPRIOTXT) %>% 
    filter(Comment != "") %>% 
